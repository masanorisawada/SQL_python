...
class DataAccess:
    ...
    def get_distance(self):
        ...
        db = DB(Var.hostname, Var.port, Var.dbname, Var.username, Var.password)
        return db.execute(query, data)
    def get_name_latitude_longitude(self):
        query = "SELECT spot_name, spot_latitude, spot_longitude FROM data_spot_impr"
        data = ()
        db = DB(Var.hostname, Var.port, Var.dbname, Var.username, Var.password)
        return db.execute(query, data)
    def get_spots_by_space_name(self, spot_name):
        query = "SELECT  spot_name, spot_latitude, spot_longitude FROM data_spot_impr WHERE spot_name = %s "
        data = (str(spot_name), )
        db = DB(Var.hostname, Var.port, Var.dbname, Var.username, Var.password)
        return db.execute(query, data)
